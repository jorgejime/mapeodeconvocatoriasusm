...
const CHART_COLORS = ['hsl(var(--primary))', 'hsl(var(--secondary))', 'hsl(var(--success))', 'hsl(var(--warning))', 'hsl(var(--danger))', '#8884d8', '#82ca9d'];
export default function Estadisticas() {
  ...
  const fetchStats = async () => {
    try {
      const {
        ...
        error
      } = await supabase.from("convocatorias").select("*");
  ...
  const calculateStats = (convocatorias: Convocatoria[]) => {
    ...
    const porSector = Object.entries(sectorCount).map(([name, value]) => ({
      name,
      value
    })).sort((a, b) => b.value - a.value).slice(0, 10);

    // Por mes de creación
    const mesCount: Record<string, {
      convocatorias: number;
    }> = {};
    convocatorias.forEach(c => {
    ...
    const porMes = Object.entries(mesCount).map(([name, data]) => ({
      name,
      ...data
    })).sort((a, b) => {
    ...
    const porEntidad = Object.entries(entidadCount).map(([name, value]) => ({
      name,
      value
    ...
    const currencyStatsAbiertas: Record<string, {
      total: number;
      count: number;
      values: number[];
    }> = {};
    const montoTotalAbierto = {
      cop: 0,
      usd: 0,
      eur: 0,
      otros: 0
      ...
      if (c.valor && c.tipo_moneda) {
        ...
        if (c.estado_convocatoria === "Abierta") {
          if (!currencyStatsAbiertas[currency]) {
            currencyStatsAbiertas[currency] = {
              total: 0,
              count: 0,
              values: []
            };
  ...
  return <div className="min-h-screen bg-background p-8 space-y-12">
      {/* Header - Minimal */}
      ...
      <div className="grid grid-cols-12 gap-8 max-w-7xl">
            ...
            <div className="space-y-8">
              ...
              <div className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={stats.porEstado} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={2} dataKey="value">
                      {stats.porEstado.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                  </PieChart>
                </ResponsiveContainer>
              </div>
              <div className="flex gap-4 text-sm">
                {stats.porEstado.map(item => <div key={item.name} className="flex items-center gap-2">
          ...
          <CardContent className="p-8">
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                {stats.porSector.slice(0, 5).map((sector, index) => <div key={sector.name} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3 min-w-20">
                      ...
                      width: `${sector.value / Math.max(...stats.porSector.map(s => s.value)) * 100}%`
                    }} />
                      </div>
        ...
        {stats.currencyStatsAbiertas.length > 0 && <Card className="col-span-12 border-0 shadow-sm bg-success/5 border-success/20">
            <CardContent className="p-8">
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {stats.currencyStatsAbiertas.map(currency => <div key={currency.currency} className="space-y-4">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Promedio: {currency.currency === 'COP' ? `$${(currency.average / 1000000).toFixed(1)}M` : currency.currency === 'USD' ? `$${(currency.average / 1000).toFixed(0)}K` : `€${(currency.average / 1000).toFixed(0)}K`}
                          </div>
                        </div>
                      </div>
                    </div>)}
